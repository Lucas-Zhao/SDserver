...
export const Abilities: import("../sim/dex-abilities").AbilityDataTable = {
	...
	windwalker: {
		onStart(pokemon) {
			if (pokemon.side.sideConditions["tailwind"]) {
				pokemon.side.sideConditions["tailwind"].duration = 5;
				return;
			} else {
			pokemon.side.addSideCondition('tailwind')
			this.add('-activate', pokemon, 'ability: Wind Walker');
			if (pokemon.side.sideConditions["tailwind"]) {
				pokemon.side.sideConditions["tailwind"].duration = 5;
				return;
			}
	...
	gravitygenerator: {
		onStart(pokemon) {
			if (pokemon.side.sideConditions["gravity"]) {
				pokemon.side.removeSideCondition('gravity')
				return;
			} else {
			pokemon.side.addSideCondition('gravity')
			this.add('-activate', pokemon, 'ability: Gravity Generator');
			if (pokemon.side.sideConditions["gravity"]) {
				pokemon.side.sideConditions["gravity"].duration = 5;
				return;
			}
		}
		},
	...
	cleanfreak: {
		onStart(pokemon) {
			...
			sideConditions.forEach((condition) => {
				...
				this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Sigil\'s Storm', '[of] ' + pokemon);
			})
		},
		flags: {},
		name: "Clean Freak",
